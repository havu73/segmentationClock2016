...
void delete_file(int fd){
	char buf [1024];
	char path_name [128];
	strcpy(path_name, "/proc/self/fd/");
	char fdbuf [13];
	sprintf(fdbuf, "%d", fd);
	strcat(path_name, fdbuf);
	strcat(path_name, "/");
	ssize_t len = readlink(path_name, buf, sizeof(buf)-1);
	if (len != -1){
		buf[len] = '\0';
		if (remove(buf) != 0){
			cout << "error deleting file" << endl;
		}else {
			cout << "file successfully deleteed" << endl;
		}
	}
}
...
void print_good_set (double parameters[], double score) {
    if (ip.print_good_sets && score <= ip.good_set_threshold) {
        for (int i = 0; i < (ip.num_dims - 1); i++) {
            ip.good_sets_stream << parameters[i] << ",";
        }
        ip.good_sets_stream << parameters[ip.num_dims - 1];
...
/* read_pipe_int writes an integer from the given pipe
	...
	todo:
*/
void read_pipe_double (int fd, double* address) {
