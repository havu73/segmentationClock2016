	...
	if (num_args > 1) { // If arguments were given (the 0th argument is the program name)
		for (int i = 1; i < num_args; i += 2) { // Iterate through each argument pair (if an argument does not have an accompanying value, i-- should be called when found)
			...
			if (option_set(option, "-r", "--ranges-file")) {
                ...
                if (ip.good_set_threshold < 0) 				{
                    usage("The threshold for a good set must be a possible score. Set -G or --good-set-threshold to be no less than 0.");
                }
            }else if (option_set(option, "-t", "--print-good-set")) {
                ensure_nonempty(option, value);
                strcpy(ip.good_sets_file, value);
...
void init_sim_args (input_params& ip) {
	...
	ip.sim_args[0] = copy_str("stochastic");
...
void set_printing_precision(input_params& ip){
	(ip.good_sets_stream).precision(ip.printing_precision);
}
