...
int main (int argc, char** argv) {
	...
	init_verbosity(ip);
	init_sim_args(ip);
	set_printing_precision(ip);
	// Create a file to print the good sets:
	if (ip.print_good_sets){
		open_file(&ip.good_sets_stream, ip.good_sets_file, true);
	}
...
void usage (const char* message) {
	...
	cout << "-G, --good-set-threshold [float]      : the threshold of score for good set. If a set scores less than or equal to the threshold (score 0 : good, score 1: bad), the set gets printed to file. Default: 0 (only print perfect sets)" << endl;
	cout << "-t, --print-good-set     [filename]   : the relative filename of the file to print good sets out" << endl;
